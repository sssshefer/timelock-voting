...
contract TimelockVoting {
    ...
    function execute(
        ...
        require(
            Transactions[txCach].queued,
            "You need to queue the transaction first"
        );
        require(_execTimestamp < block.timestamp, "not ready to be called yet");
        require(
        ...
        (bool success, bytes memory response) = _to.call{value: msg.value}(
            data
        );
        require(
            success,
            "The transaction execution occured with error. The transaction is deleted"
        );
    ...
    function discard(bytes32 _txCash) external onlyOwner {
        require(
            Transactions[_txCash].queued,
            "The transaction is not in the queue"
        );
...
contract OtherContract {
    ...
    function someTestFunc(string memory _message) external payable {
        callCount++;
        message = _message;
    }

    receive() external payable {
        message = "Filled with money";
    }
}
